...
describe("Polygon fork three-leg execution", function () {
  ...
  const BALANCER_LEG1_OUT = ethers.BigNumber.from("405700159370346");
  const BALANCER_LEG2_OUT = ethers.BigNumber.from("9910152638245722736");
  const BALANCER_LEG3_OUT = ethers.BigNumber.from("1002583");
  const BALANCER_INDEX_POOL_ID =
  ...
  function encodeBalancerRoute() {
    ...
    return ethers.utils.defaultAbiCoder.encode(
      [legType],
      [[
        [2, USDC_E, WETH, BALANCER_LEG1_OUT, V3_FEE_500],
        [3, WETH, WPOL, BALANCER_LEG2_OUT, BALANCER_INDEX_POOL_ID],
        [2, WPOL, USDC_E, BALANCER_LEG3_OUT, V3_FEE_500]
      ]]
    );
  }
  ...
  it("executes the historical Balancer USDC.e -> WETH -> WPOL -> USDC.e route", async function () {
    ...
    console.log("Balancer fork gasUsed:", receipt.gasUsed.toString());
    console.log("Balancer bot USDC.e before:", before.toString());
